...
import CartProductMore from "@/components/share/CartProductMore";
...
export async function generateMetadata({
  ...
  const dataHome = await fetchWithToken(
    `${ENDPOINT.GET_DANH_MUC_TRANG_CHU}?${searchParams}&filters[slug][$eq]=${slug}`
  );
...
const page = async ({ params }: { params: { slug: string } }) => {
  ...
  const products = dataHome?.data[0]?.attributes?.san_phams;
      ...
      <div className="container">
        ...
        {products?.data?.map((product: any, index: number) => (
            <div key={product.id} className="col-span-6 md:col-span-3">
              <CartProductMore data={product} index={index} />
            </div>
          ))}
        </div>
      </div>
