...
interface CartProductMoreProps {
  data: any;
  index?: number;
}

const CartProductMore: React.FC<CartProductMoreProps> = ({ data, index = 99 }) => {
  ...
  const salePrice =
    data?.attributes?.size[0]?.price *
      ((100 - data?.attributes.percentSale) / 100) || 0;

  const originalPrice = data?.attributes?.size[0]?.price || 0;
  useEffect(() => {
  ...
  const getOptimizedImage = () => {
    const imgData = data?.attributes?.images?.data?.[0]?.attributes;
  ...
  return (
    <>
      <Link href={`/san-pham/${data?.attributes?.slug}`}>
